...
void generate_true_cost_map( TERRAIN *tt, unsigned int seed )
{
  int i, j;
  // int ir;
  int ir2;
  ...
  terrain_indices( tt, 3.0f, 3.0f, &ixc2, &iyc2, NULL );
  ...
  if ( first_time )
    {
      ...
      /*
      printf( "ROOT NODE\n" );
      printf( "%d %d: %g %g %g %g %g\n", an->parent->id, an->parent->depth,
	      an->parent->cost_from_start,
	      an->parent->one_step_cost,  an->parent->terrain_cost,
	      an->parent->cost_to_go, an->parent->priority );
      */
